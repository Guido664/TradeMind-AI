...
import { calculateLorentzianSignal } from './lorentzianService';
...
const fetchStrategies = [
    ...
    // 2. Thingproxy: Fast and reliable alternative
    async (url: string, signal: AbortSignal) => {
        const res = await fetch(`https://thingproxy.freeboard.io/fetch/${url}`, { signal });
        if (!res.ok) throw new Error(`Thingproxy status: ${res.status}`);
        return res.json();
    },
    // 3. CodeTabs: Good alternative
    async (url: string, signal: AbortSignal) => {
    ...
    for (const strategy of fetchStrategies) {
        try {
            ...
            const result = await Promise.race([
                strategy(targetUrl, controller.signal),
...
export const searchSymbols = async (query: string): Promise<SearchResult[]> => {
  ...
  // 2. Try TradingView Search API (Enhancement)
  try {
      const url = `/api/tradingview/symbol_search/?text=${encodeURIComponent(cleanQuery)}&hl=en&type=&exchange=&lang=en&domain=production`;
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
          const apiResults = data
              ...
              .map((q: any) => ({
                  symbol: q.symbol,
                  shortname: q.description || q.symbol,
      ...
      console.warn("Search API failed, using manual fallback only.", e);
  }
...
const fetchYahooData = async (symbol: string, timeframe: TimeFrame): Promise<{data: OHLCData[], usedTicker: string} | null> => {
    ...
    const tryFetch = async (tickerToTry: string) => {
        try {
            const url = `/api/yahoo/v8/finance/chart/${tickerToTry}?range=${range}&interval=${interval}&includePrePost=false`;
...
export const generateMarketData = async (ticker: string, timeframe: TimeFrame): Promise<AnalysisResult> => {
  ...
  const mlSignal = calculateLorentzianSignal(mergedData);
